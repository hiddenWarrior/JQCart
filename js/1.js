...
layoutMaker.prototype.makeDiv=function(item){
	...
	var div = "<div class='product'>"+i["name"]+","+i["quantity"]+","+i["price"]+"<input type='Button' id='edit' value='Edit'/><input type='Button' id='delete' value='X' /></div>"//.replace("%name",i.name).replace("%quantity",i.quantity).replace("%price",i.price);
	return div;
}
